...
<div class="page-header-wrap">
    <div class="breadcrumb">
        <a href="{{ route('admin.products') }}" style="text-decoration:none; color:inherit;">Products</a> &nbsp;/&nbsp; Edit Commodity
    </div>
    <div style="display:flex; justify-content:space-between; align-items:center;">
        <h1 class="page-title">Edit Commodity: {{ $product->name }}</h1>
    </div>
</div>
...
<div class="card" style="max-width: 800px;">
    <form action="{{ route('admin.products.update', $product->id) }}" method="POST">
        ...
        <div style="display:grid; grid-template-columns:1fr 1fr; gap:20px;">
            ...
            <div class="field">
                ...
                <select name="unit" required>
                    ...
                    <option value="ton" {{ old('unit', $product->unit) == 'ton' ? 'selected' : '' }}>Tons</option>
            ...
            <div class="field">
                ...
                <input type="number" name="min_order_qty" value="{{ old('min_order_qty', $product->min_order_qty) }}" required>
            ...
            <div class="field">
                ...
                <select name="grade" required>
                    <option value="A+" {{ old('grade', $product->grade) == 'A+' ? 'selected' : '' }}>Grade A+ (Export Quality)</option>
                    <option value="A" {{ old('grade', $product->grade) == 'A' ? 'selected' : '' }}>Grade A</option>
                    <option value="B" {{ old('grade', $product->grade) == 'B' ? 'selected' : '' }}>Grade B</option>
                    <option value="C" {{ old('grade', $product->grade) == 'C' ? 'selected' : '' }}>Grade C</option>
                </select>
            ...
            <div class="field">
                ...
                <input type="text" name="origin" value="{{ old('origin', $product->origin) }}" required>
            ...
            <div class="field">
                ...
                <select name="stock_status" required>
                    ...
                    <option value="SOLD_OUT" {{ old('stock_status', $product->stock_status) == 'SOLD_OUT' ? 'selected' : '' }}>Sold Out</option>
                </select>
        ...
        <div class="field">
            ...
            <textarea name="description" rows="3" style="width:100%; padding:12px 14px; border-radius:10px; border:1.5px solid var(--line-strong); font-family:inherit; font-size:14.5px;">{{ old('description', $product->description) }}</textarea>
        ...
        <div style="display:flex; gap:20px; margin-top:16px; margin-bottom:24px; padding:16px; background:var(--bg); border-radius:8px;">
            <label style="display:flex; align-items:center; gap:8px; font-size:14px; font-weight:600; cursor:pointer;">
                <input type="checkbox" name="is_gst_available" value="1" {{ old('is_gst_available', $product->is_gst_available) ? 'checked' : '' }}>
                GST Invoice Available
            </label>
            <label style="display:flex; align-items:center; gap:8px; font-size:14px; font-weight:600; cursor:pointer;">
                <input type="checkbox" name="is_delivery_available" value="1" {{ old('is_delivery_available', $product->is_delivery_available) ? 'checked' : '' }}>
                Delivery Available
            </label>
            <label style="display:flex; align-items:center; gap:8px; font-size:14px; font-weight:600; cursor:pointer;">
                <input type="checkbox" name="is_pickup_available" value="1" {{ old('is_pickup_available', $product->is_pickup_available) ? 'checked' : '' }}>
                Pickup Available
            </label>
        </div>
        
        <div style="display:flex; gap:12px;">
            <button type="submit" class="btn btn-primary" style="padding: 12px 32px;">Update Commodity</button>
